...
function universh_post_thumb( $width=300, $height='', $html=true, $crop = true ) {		
  global $post;
  if( !has_post_thumbnail() ) return false;

  $fullsize = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' );
  $url = universh_image_resize( $fullsize[0], $width, $height, $crop, 'c', false );

  if($html){
    echo '<img src="'.esc_url($url).'" alt="'.esc_attr(get_the_title(get_post_thumbnail_id( $post->ID ))).'" class="img-responsive" />';
  }else{
    return '<img src="'.esc_url($url).'" alt="'.esc_attr(get_the_title(get_post_thumbnail_id( $post->ID ))).'" class="img-responsive" />';
  }
}
...
function add_image_thumb($content)	//add thumb in the begining of the post
{
	...
	if($img['show'] != false) {
		if($img['src'] != "") {
			...
			$img = '<a href="#" class="image-overlay-1 main-image"><span><img src="'.esc_url($img['src']).'" class="img-responsive" alt="'.esc_attr(get_the_title()).'"/></span></a>';
			return $img." ".$content;
		} else {
			return $content;
		}
...
add_action('delete_attachment', 'universh_delete_resized_images');

function universh_image_resize($url, $width=null, $height=null, $crop=true, $align='c', $retina=false) {

  global $wpdb;

  // Get common vars (func_get_args() only get specified values)
  $common = universh_common_info($url, $width, $height, $crop, $align, $retina);
...
function universh_common_info($url, $width, $height, $crop, $align, $retina) {
...
function universh_delete_resized_images($post_id) {
